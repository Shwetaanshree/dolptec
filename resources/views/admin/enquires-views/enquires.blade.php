                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form class="enquiry-form" action="{{ route('enquire.delete.multiple') }}" method="POST">
                                @csrf
                                @method('delete')
                                ...
                                <table id="enquire_table" class="table table-responsive table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><input type="checkbox" name="enquiry_ids[]" value="{{ $item->id }}"></td>
                                            <td>{{$key++}}</td>
                                            <td>{{ $item->name}}</td>
                                            <td>{{ $item->email }}</td>
                                            <td>{{ $item->phone }}</td>
                                            ...
                                            <td>                                           
                                                ...
                                                <td>
                                               <form action="{{ route('enquire.delete') }}" method="POST">
                                                   @csrf
                                                  @method('delete')
...
<script>
    document.getElementById('select_all').onclick = function() {
        var checkboxes = document.getElementsByName('enquiry_ids[]');
