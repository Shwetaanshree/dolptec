...
class DownloadBrochureController extends Controller
{
    ...
    {
        $leadIds = $request->input('lead_ids'); // Adjust based on your form input

        if (!empty($leadIds)) {
            DownloadBrochure::whereIn('id', $leadIds)->delete();
        }

        return redirect()->route('leadsDashboard')->with('success', 'Selected leads have been deleted.');
    }
    ...
    public function store(Request $request)
    {
        ...
        // Send email (you may need to configure mail settings in .env and mail.php config)
        $mailData = [
            'name' => $request->input('name'),
            'email' => $request->input('email'),
            'phone' => $request->input('phone'),
        ...
        $filePath = public_path('assets/pdf/' . $pdfFile);
    ...
    {
        $leads = DownloadBrochure::all(); // Fetch all leads

        return view('admin.download-brochure.downloads-leads', compact('leads'));
    }
}
